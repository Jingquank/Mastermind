...
describe('maskSource', () => {
  ...
  it('falls back to an alternate sentinel pool on collision', () => {
    ...
    expect(sentinels.neutral[0].charCodeAt(0)).toBeGreaterThanOrEqual(0xe010)
  })

  it('alternates neutral chars so adjacent atoms stay distinguishable', () => {
    const text = '{>>one<<}{>>two<<}'
    const { masked, sentinels } = maskSource(text, scan(text))
    expect(masked).toContain(sentinels.neutral[0])
    expect(masked).toContain(sentinels.neutral[1])
  })

  it('pickSentinels throws only when every pool is exhausted', () => {
    let everything = ''
    for (const base of [0xe000, 0xe010, 0xe020, 0xe030, 0xe040]) {
      for (let off = 0; off < 8; off++) everything += String.fromCharCode(base + off)
    }
