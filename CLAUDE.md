...
- **`src/ui/`** — React 19 + Vite SPA, Zustand stores. Three views over one buffer (`Cmd+E` cycles): **Reading** (`modes/reading/Renderer.tsx`, rendered markdown with CriticMarkup as a visual diff), **Editing** (`modes/editing/MilkdownEditor.tsx`, WYSIWYG — opening+saving an unchanged file must be byte-identical), **Source** (`modes/source/SourceEditor.tsx`, CodeMirror). `review/` holds the comment rail, hover accept/reject, find bar, rounds panel, and the inline-suggestion proposal store.
...
- **Byte-exact round-trips.** Open→save with no edits must reproduce the file byte-for-byte (EOL and all). Accept/reject of a mark on an unedited document is an exact source splice, not a re-render. See `milkdown-roundtrip.test.ts`, `critic-resolve.test.ts`, `editing-resolve.test.ts`.
